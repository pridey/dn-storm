...
optparse = OptionParser.new do |opts|
  ...
  options[:storm_addr] = nil
  opts.on( '-s', '--storm-addr IP_ADDR', 'IP_ADDR of the storm server' ) do |storm_addr|
    ...
    options[:storm_addr] = storm_addr.gsub(/^=/,'')
  end
...
if ip_required && !options[:storm_addr]
  print "ERROR; server IP address must be supplied for vagrant commands\n"
  print optparse
  exit 1
elsif options[:storm_addr] && !(options[:storm_addr] =~ Resolv::IPv4::Regex)
  print "ERROR; input server IP address '#{options[:storm_addr]}' is not a valid IP address"
  exit 2
end

if options[:storm_url] && !(options[:storm_url] =~ URI::regexp)
  print "ERROR; input Solr URL '#{options[:storm_url]}' is not a valid URL\n"
  exit 3
end
...
Vagrant.configure("2") do |config|
  ...
  #config.vm.network "forwarded_port", guest: "#{options[:storm_addr]}", host: "#{options[:storm_addr]}"

  # Create a private network, which allows host-only access to the machine
  # using a specific IP.
  config.vm.network "private_network", ip: "#{options[:storm_addr]}"
  ...
  config.vm.define "#{options[:storm_addr]}"
  storm_addr_array = "#{options[:storm_addr]}".split(/,\w*/)
  ...
  config.vm.provision "ansible" do |ansible|
    ...
    # the command-line (eg. "https://10.0.2.2/apache-storm-1.0.2.tar.gz.tar.gz")
